...
const { modelNameFromMetadata, objectNamesFromMetadata } = require('./print-name.js');
...
function parseMaterials(meta) {
  ...
  // Keep empty entries: their index still corresponds to a physical tool slot.
  return raw.split(sep).map((material) => material.trim() || null);
}
...
const ANALYSIS_VERSION = 6; // bump when the analysis shape changes so stale caches are rebuilt
...
function decodeTextMetadata(gcodeText) {
  const meta = {};
...
// Analyze either Prusa binary G-code or the plain-text .gcode returned by Connect.
function analyzeBgcode(fileBuf) {
  if (!Buffer.isBuffer(fileBuf) || fileBuf.length === 0) throw new Error('empty G-code response');
  const binary = fileBuf.toString('ascii', 0, 4) === 'GCDE';
  const gcodeText = binary ? decodeGcodeText(fileBuf) : fileBuf.toString('utf8');
  if (!binary && (gcodeText.includes('\0') ||
      !/^\s*(?:N\d+\s+)?(?:[GMT]\d+|T\d+)(?:\s|$)/im.test(gcodeText))) {
    throw new Error('response is not plausible plain-text G-code');
  }
  const a = buildTimeline(gcodeText);
  const meta = binary ? decodeMetadata(fileBuf) : decodeTextMetadata(gcodeText);
  a.materials = parseMaterials(meta);       // 0-indexed by tool
  // Keep only the compact model-name subset from objects_info. The raw field can
  // contain large bed polygons and is intentionally not persisted in the cache.
  a.objectNames = objectNamesFromMetadata(meta);
  a.modelName = modelNameFromMetadata(meta);
...
module.exports = { parseMaterials, buildTimeline, mapLive, analyzeBgcode, materialFor, ANALYSIS_VERSION };
