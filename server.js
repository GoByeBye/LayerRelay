...
const { canWrite } = require('./access-control.js');
...
const { AutomationLease } = require('./automation-lease.js');
const { pruneAnalysisCache } = require('./cache-retention.js');
const { normalizeOverlayHost } = require('./overlay-host.js');
...
const overlayHost = normalizeOverlayHost(cfg.overlayHost);
const automationLease = new AutomationLease({ durationMs: cfg.automationLeaseMs });
...
const MESSAGE_FILE = path.join(CACHE_DIR, 'message.json');
const MESSAGE_MAX_LEN = 200;
const ANNOUNCE_FILE = path.join(CACHE_DIR, 'announce.json');
const ANNOUNCE_MAX_LEN = 280;
const ANNOUNCE_KEEP = 8;
...
// Sticky overlay message (e.g. "brb", "chat is muted for X"): set via POST /api/message,
// stays on screen across polls/restarts until cleared via DELETE /api/message.
let message = null; // { text, updatedAt } | null
{
  const saved = readJsonWithBackup(MESSAGE_FILE, null);
  if (saved && saved.text) message = saved;
}

// Configurable stream-host feed: short narration lines posted via POST /api/announce, shown in
// the overlay's feed box (newest + one line of history). Kept small and persisted so a
// server restart mid-stream doesn't blank the box.
let announce = []; // [{ text, at }] oldest first
{
  const saved = readJsonWithBackup(ANNOUNCE_FILE, null);
  if (Array.isArray(saved)) announce = saved.filter((a) => a && a.text).slice(-ANNOUNCE_KEEP);
}
...
function requireWriteAccess(req, res, next) {
  ...
  const supplied = req.get('x-api-token') || (bearer && bearer[1]);
  const access = {
  ...
  if (canWrite(access)) return next();
  res.set('WWW-Authenticate', 'Bearer');
  return res.status(401).json({ error: 'authentication required' });
}

app.use((req, res, next) => {
  if (req.method === 'POST' || req.method === 'DELETE') return requireWriteAccess(req, res, next);
  next();
});
...
app.get('/api/state', (_req, res) => {
  ...
  const automationStatus = automationLease.status();
  res.json({
    ...out,
    online,
    staleSec,
    ...
    overlayHost: {
      ...overlayHost,
      // Runtime presence is a short, non-persistent lease. Static config and old
      // announcements can never claim that automation is currently operating.
      automationRunning: automationStatus.running,
      automationExpiresAt: automationStatus.expiresAt,
    },
    // Stream-host feed, newest last (the overlay shows the last one big + one history line).
    announce: announce.slice(-3),
...
// AUTO MODE is a fail-closed runtime lease: an active operator refreshes it every
// 15 seconds, it expires automatically, and a server restart always clears it.
app.get('/api/automation/status', (_req, res) => {
  res.json(automationLease.status());
});
function requireAutomationIntent(req, res, next) {
  if (req.get('x-automation-heartbeat') === '1') return next();
  return res.status(400).json({ error: 'X-Automation-Heartbeat: 1 is required' });
}
app.post('/api/automation/heartbeat', requireAutomationIntent, (_req, res) => {
  res.json(automationLease.heartbeat());
});
app.delete('/api/automation/heartbeat', requireAutomationIntent, (_req, res) => {
  res.json(automationLease.clear());
});
// Sticky overlay message: GET current, POST { text } to set, DELETE to clear.
// Persisted to disk so a server restart doesn't drop it before it's explicitly cleared.
app.get('/api/message', (_req, res) => {
  res.json(message || { text: null });
});
app.post('/api/message', express.json({ limit: '4kb' }), (req, res) => {
  const text = typeof req.body?.text === 'string' ? req.body.text.trim() : '';
  if (!text) return res.status(400).json({ error: 'text is required' });
  if (text.length > MESSAGE_MAX_LEN) return res.status(400).json({ error: `text exceeds ${MESSAGE_MAX_LEN} chars` });
  const nextMessage = { text, updatedAt: Math.floor(Date.now() / 1000) };
  writeJsonAtomic(MESSAGE_FILE, nextMessage);
  message = nextMessage;
  res.json(message);
});
app.delete('/api/message', (_req, res) => {
  writeJsonAtomic(MESSAGE_FILE, { text: null, updatedAt: Math.floor(Date.now() / 1000) });
  message = null;
  res.json({ text: null });
});
// Stream-host feed: GET the history, POST { text } to add a line, DELETE to clear.
app.get('/api/announce', (_req, res) => {
  res.json(announce);
});
app.post('/api/announce', express.json({ limit: '4kb' }), (req, res) => {
  const text = typeof req.body?.text === 'string' ? req.body.text.trim() : '';
  if (!text) return res.status(400).json({ error: 'text is required' });
  if (text.length > ANNOUNCE_MAX_LEN) return res.status(400).json({ error: `text exceeds ${ANNOUNCE_MAX_LEN} chars` });
  const nextAnnounce = [...announce, { text, at: Math.floor(Date.now() / 1000) }].slice(-ANNOUNCE_KEEP);
  writeJsonAtomic(ANNOUNCE_FILE, nextAnnounce);
  announce = nextAnnounce;
  res.json(announce[announce.length - 1]);
});
app.delete('/api/announce', (_req, res) => {
  writeJsonAtomic(ANNOUNCE_FILE, []);
  announce = [];
  res.json([]);
});
...
const listenHost = cleanSlotText(cfg.listenHost || cfg.bindHost, 255) || '127.0.0.1';
const httpServer = app.listen(listenPort, listenHost, () => {
  console.log(`3d-livestream overlay: http://${listenHost}:${listenPort}/`);
  ...
  if (!cfg.apiToken && listenHost !== '127.0.0.1' && listenHost !== '::1' && listenHost !== 'localhost') {
    console.warn('security warning: remote write requests are disabled until apiToken is configured');
  }
  pollLoop();
  if (connectEnabled) {
