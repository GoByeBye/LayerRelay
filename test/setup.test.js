...
test('setup honors LAYER_RELAY_CONFIG for an external private path', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'layer-relay-setup-'));
  ...
  const result = spawnSync(process.execPath, [setupScript, '--non-interactive'], {
    ...
    env: {
      ...process.env,
      CONFIG_PATH: '',
      LAYER_RELAY_CONFIG: configPath,
    },
    encoding: 'utf8',
  });
  assert.equal(result.status, 0, `${result.stdout}\n${result.stderr}`);
  const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  assert.equal(config.useConnect, true);
