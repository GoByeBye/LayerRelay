...
function tempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), '3d-livestream-config-'));
...
test('supports an environment-only container configuration', () => {
  ...
  assert.equal(runtime.config.apiToken, '0123456789abcdef0123456789abcdef'); // gitleaks:allow -- test fixture
});
...
test('requires strong write tokens and RTSP camera URLs', () => {
  assert.throws(
    () => validateConfig({
      printerHost: 'printer.local',
      username: 'maker',
...
test('rejects schema typos, unsafe poll cadences, and non-string or padded tokens', () => {
  const base = {
    printerHost: 'printer.local',
    username: 'maker',
    password: 'secret',
  ...
  for (const [patch, pattern] of [
    ...
    [{ automationLeaseMs: 5000 }, /automationLeaseMs must be an integer/],
    [{ apiToken: true }, /apiToken must be a string/],
    [{ apiToken: ` ${'a'.repeat(32)}` }, /leading or trailing whitespace/],
    ...
    [{ overlayHost: { icon: '/assets/../config.json' } }, /traversal-free/],
...
test('rejects malformed JSON with the path and no file contents', () => {
  ...
  fs.writeFileSync(configPath, '{"apiToken": TOP_SECRET_TOKEN,');
  assert.throws(
