...
function createRuntime({ height = 1080, search = '', storedRoom = null, cameraStatus, stateFailure = false } = {}) {
  const imageRequests = [];
  const fetchRequests = [];
  ...
  if (storedRoom != null) storage.set('3d-livestream.room-visible', storedRoom ? '1' : '0');
  let timerId = 0;
...
test('room temperature is opt-in, persisted per browser, and URL-overridable', () => {
  ...
  assert.equal(hidden.storage.get('3d-livestream.room-visible'), '1');
...
test('announcement feed renders configured host identity and lazy same-origin artwork', () => {
  const runtime = createRuntime({ height: 1080 });
  const host = {
    avatar: 'CX',
    name: '<PRINT BOT>',
  ...
  runtime.api.S.data = {
    state: 'PRINTING', online: true, staleSec: 0, toolCount: 8,
    overlayHost: host, announce: [],
  };
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-avatar-image').getAttribute('src'), null);

  runtime.api.S.data.announce = [{ text: 'Relay is live', at: 1_700_000_000 }];
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-avatar-text').textContent, 'CX');
  assert.equal(runtime.elements.get('feed-name').textContent, '<PRINT BOT>');
  assert.equal(runtime.elements.get('feed-badge').textContent, 'BETA');
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), false);
  assert.equal(runtime.elements.get('feed').classList.contains('show'), true);
  assert.equal(runtime.elements.get('feed-avatar').classList.contains('has-image'), true);
  assert.equal(runtime.elements.get('feed-avatar-image').classList.contains('pet-atlas'), true);
  assert.equal(runtime.elements.get('feed-avatar-image').getAttribute('src'), host.icon);

  runtime.elements.get('feed-avatar-image').onerror();
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-avatar').classList.contains('has-image'), false);

  runtime.api.S.data.overlayHost = { ...host, icon: '/assets/replacement.webp', iconMode: 'image' };
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-avatar').classList.contains('has-image'), true);
  assert.equal(runtime.elements.get('feed-avatar-image').classList.contains('image-icon'), true);

  runtime.api.S.data.overlayHost.automationRunning = true;
  runtime.api.S.data.overlayHost.automationExpiresAt = Date.now() + 60000;
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-mode-badge').textContent, 'AUTO MODE');
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), true);

  runtime.api.S.failures = 3;
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), false);
  runtime.api.S.failures = 0;

  runtime.api.S.data.overlayHost.automationExpiresAt = Date.now() - 1;
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), false);

  runtime.api.S.data.overlayHost.automationExpiresAt = Date.now() + 60000;

  runtime.api.S.data.state = 'IDLE';
  runtime.api.render();
  assert.equal(runtime.elements.get('feed-badge').textContent, 'BETA');
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), true);
});

test('announcement host has neutral text fallbacks when an older backend omits configuration', () => {
  const runtime = createRuntime({ height: 420 });
  runtime.api.S.data = {
    state: 'PRINTING', online: true, staleSec: 0, toolCount: 8,
    announce: [{ text: 'Fallback identity', at: 1_700_000_000 }],
  };
  runtime.api.render();

  assert.equal(runtime.elements.get('feed-avatar-text').textContent, '3D');
  assert.equal(runtime.elements.get('feed-name').textContent, 'PRINT HOST');
  assert.equal(runtime.elements.get('feed-badge').textContent, 'LIVE');
  assert.equal(runtime.elements.get('feed-mode-badge').classList.contains('show'), false);
  assert.equal(runtime.elements.get('feed-avatar').classList.contains('has-image'), false);
});
