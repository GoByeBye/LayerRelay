...
const DEFAULT_CONFIG = Object.freeze({
  ...
  port: 8787,
  sourceCodeUrl: 'https://github.com/GoByeBye/3d-livestream',
  pollIntervalMs: 2000,
...
const ENV_OVERRIDES = Object.freeze({
  ...
  OVERLAY_API_TOKEN: ['apiToken', 'secret'],
  SOURCE_CODE_URL: ['sourceCodeUrl', 'string'],
});

const KNOWN_CONFIG_KEYS = new Set([
  '$schema',
  'printerHost', 'username', 'password', 'listenHost', 'bindHost', 'port', 'sourceCodeUrl', 'pollIntervalMs',
  'analysisCacheMaxEntries', 'analysisCacheMaxBytes', 'printNameOverrides', 'overlayHost',
  ...
  'automationLeaseMs', 'lastStateWriteMs', 'maxPrinterJsonBytes',
  'maxPrinterResponseBytes', 'debugMaxBytes', 'apiToken',
]);
...
const INTEGER_RANGES = Object.freeze({
  ...
  connectPollMs: [1000, Number.MAX_SAFE_INTEGER],
  netatmoPollMs: [60000, Number.MAX_SAFE_INTEGER],
  automationLeaseMs: [15000, 300000],
...
const OPTIONAL_STRING_KEYS = Object.freeze([
  '$schema', 'bindHost', 'cameraRtspUrl', 'cameraFfmpegPath', 'connectPrinterUuid',
  'connectClientId', 'connectRefreshToken', 'netatmoClientId', 'netatmoClientSecret',
  'netatmoRefreshToken', 'apiToken',
]);
...
function applyEnvironment(config, env) {
  ...
  for (const [name, [key, type]] of Object.entries(ENV_OVERRIDES)) {
    const raw = firstEnvironmentValue(env, name, `LIVESTREAM_${name}`);
...
function validateConfig(config, { requirePrinter = true } = {}) {
  ...
  if (typeof config.apiToken === 'string') {
    if (config.apiToken !== config.apiToken.trim()) {
      errors.push('apiToken must not have leading or trailing whitespace');
    }
    if (/[\r\n]/.test(config.apiToken)) errors.push('apiToken must not contain line breaks');
    if (config.apiToken && config.apiToken.length < 32) {
      errors.push('apiToken must contain at least 32 characters when configured');
    }
  }
  ...
  if (config.overlayHost != null) {
    if (!plainObject(config.overlayHost)) {
      errors.push('overlayHost must be an object');
    } else {
      const limits = { avatar: 8, name: 40, badge: 24, modeBadge: 24 };
      for (const key of Object.keys(config.overlayHost)) {
        if (![...Object.keys(limits), 'icon', 'iconMode'].includes(key)) errors.push(`unknown overlayHost setting: ${key}`);
      }
      for (const [key, maximum] of Object.entries(limits)) {
        const value = config.overlayHost[key];
        if (value != null && (typeof value !== 'string' || value.length > maximum)) {
          errors.push(`overlayHost.${key} must be a string of at most ${maximum} characters`);
        }
      }
      const icon = config.overlayHost.icon;
      if (icon != null && (typeof icon !== 'string' ||
          !/^\/assets\/[A-Za-z0-9][A-Za-z0-9._/-]*$/.test(icon) || icon.includes('..') || icon.includes('\\'))) {
        errors.push('overlayHost.icon must be null or a traversal-free same-origin /assets/ path');
      }
      if (config.overlayHost.iconMode != null && !['image', 'pet-atlas'].includes(config.overlayHost.iconMode)) {
        errors.push('overlayHost.iconMode must be image or pet-atlas');
      }
    }
  }
...
function loadRuntimeConfig(options = {}) {
  ...
  const configPath = resolveRuntimePath(
    rootDir,
    firstEnvironmentValue(env, 'CONFIG_PATH', 'LIVESTREAM_CONFIG').trim(),
    'config.json',
  );
  const dataDir = resolveRuntimePath(
    rootDir,
    firstEnvironmentValue(env, 'DATA_DIR', 'LIVESTREAM_DATA_DIR').trim(),
    'cache',
  );
  const fileConfig = readConfigFile(configPath, fsImpl);
  if (!fileConfig && !firstEnvironmentValue(env, 'PRINTER_HOST', 'LIVESTREAM_PRINTER_HOST')) {
    throw new Error(
  ...
  if (fileConfig && fileConfig.bindHost && !Object.hasOwn(fileConfig, 'listenHost')) {
    mergedConfig.listenHost = fileConfig.bindHost;
  }
